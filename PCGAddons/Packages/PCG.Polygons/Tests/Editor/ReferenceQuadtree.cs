...
{
	public sealed class ReferenceQuadtree
	{
		...
		public static ReferenceQuadtree Build(IList<Polygon2D> merged, float2 boundsMin, float2 boundsMax, float maxCellSize, float minCellSize, int maxDepth, Func<float2, float> sampleHeight, float maxHeightError)
		{
			var tree = new ReferenceQuadtree();
		...
		private void Subdivide(int depth, int ix, int iz)
		{
			...
			while (pending.Count > 0)
			{
				...
				if (cls == ReferenceCellClass.Outside)
					continue;

				bool canSplit = cs > MinCellSize && cell.Depth < MaxDepth;
				bool split;
				if (cls == ReferenceCellClass.Boundary)
					split = canSplit;
				else
				...
				Leaves[(cell.Depth, cell.Ix, cell.Iz)] = new QuadLeaf
				{
					...
					Ix = cell.Ix,
					Iz = cell.Iz,
					Boundary = cls == ReferenceCellClass.Boundary
				};
			}
		}

		private ReferenceCellClass Classify(float2 min, float2 max)
		{
			for (int i = 0; i < _segments.Count; i++)
			{
				...
				if (SegmentIntersectsRect(s.A, s.B, min, max))
					return ReferenceCellClass.Boundary;
			}

			float2 center = (min + max) * 0.5f;
			return RegionContains(center) ? ReferenceCellClass.Inside : ReferenceCellClass.Outside;
		}
