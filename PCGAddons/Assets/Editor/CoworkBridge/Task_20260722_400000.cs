...
public static class Task_20260722_400000
{
	...
	public static async Task<string> Run()
	{
		...
		report.Add(Render("F_CurveCross", new[]
		{
	...
	private static string Render(string label, Spline[] splines, float width)
	{
		...
		try
		{
			...
			var debugCuts = (List<Vector3[]>)Field(result, "DebugCuts");
			var debugState = (List<int>)Field(result, "DebugState");
			...
			Material wire = Mat(sh, new Color(0.02f, 0.02f, 0.03f));
			Material cutG = Mat(sh, new Color(0.1f, 0.95f, 0.2f));
			Material cutR = Mat(sh, new Color(0.98f, 0.12f, 0.08f));
			Material cutB = Mat(sh, new Color(0.2f, 0.35f, 1f));
			temp.Add(greenMat); temp.Add(blueMat); temp.Add(wire); temp.Add(cutG); temp.Add(cutR); temp.Add(cutB);

			Bounds b0 = default; bool found = false;
			Bounds rb = default; bool foundR = false;
			...
			for (int i = 0; i < debugCuts.Count; i++)
			{
				Material cm = debugState[i] == 1 ? cutR : debugState[i] == 2 ? cutB : cutG;
				Bar(debugCuts[i][0] + Vector3.up * 0.5f, debugCuts[i][1] + Vector3.up * 0.5f, 0.12f, cm, temp);
				if (debugState[i] == 1)
				{
					if (!foundR) { rb = new Bounds(debugCuts[i][0], Vector3.zero); foundR = true; } else rb.Encapsulate(debugCuts[i][0]);
					rb.Encapsulate(debugCuts[i][1]);
				}
			}
			...
			Bounds fb = foundR ? rb : b0;
			float ext = Mathf.Max(fb.size.x, fb.size.z) + 16f;
			cam.orthographicSize = Mathf.Max(ext * 0.6f, 8f);
			cam.transform.position = fb.center + Vector3.up * 200f;
	...
	private static void Bar(Vector3 a, Vector3 b, float th, Material m, List<UnityEngine.Object> temp)
	{
		float d = Vector3.Distance(a, b); if (d < 1e-4f) return;
		GameObject bar = GameObject.CreatePrimitive(PrimitiveType.Cube);
		bar.hideFlags = HideFlags.HideAndDontSave; bar.layer = Layer;
		bar.transform.position = (a + b) * 0.5f;
		bar.transform.rotation = Quaternion.LookRotation((b - a).normalized, Vector3.up);
		bar.transform.localScale = new Vector3(th, th, d);
		bar.GetComponent<MeshRenderer>().sharedMaterial = m;
		UnityEngine.Object.DestroyImmediate(bar.GetComponent<Collider>()); temp.Add(bar);
	}
	...
	private static void Spawn(SweepMeshData data, Material surface, Material wireMat, List<UnityEngine.Object> temp, ref Bounds b, ref bool found)
	{
		...
		GameObject w = new GameObject("W"); w.hideFlags = HideFlags.HideAndDontSave; w.layer = Layer; w.transform.position = Vector3.up * 0.08f;
