...
public static class Task_20260722_390000
{
	...
	public static async Task<string> Run()
	{
		...
		report.Add(Render("E_SharpV", new[] { Corner(new float3(-40, 0, 18), new float3(-2, 0, -26), new float3(40, 0, 18)) }, 4f));
		report.Add(Render("E_Cross", new[]
		{
	...
	private static string Render(string label, Spline[] splines, float width)
	{
		...
		try
		{
			...
			Material cutR = Mat(sh, new Color(0.95f, 0.15f, 0.1f));
			...
			Bounds bz = default; bool foundZ = false;
			...
			foreach (var pc in pieceList)
			{
				...
				if (st == 2)
				{
					...
					blueMeshes++;
					for (int vi = 0; vi < blueMesh.Vertices.Length; vi++) { if (!foundZ) { bz = new Bounds(blueMesh.Vertices[vi], Vector3.zero); foundZ = true; } else bz.Encapsulate(blueMesh.Vertices[vi]); }
			...
			for (int i = 0; i < debugCuts.Count; i++)
			{
				...
				Bar(debugCuts[i][0] + Vector3.up * 0.4f, debugCuts[i][1] + Vector3.up * 0.4f, 0.18f, cm, temp);
			}
			...
			Bounds fb = foundZ ? bz : b0;
			float ext = Mathf.Max(fb.size.x, fb.size.z) + 12f;
