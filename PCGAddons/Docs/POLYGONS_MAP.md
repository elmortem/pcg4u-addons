...
- `RegionToMeshNodeExecutor` (namespace `PCG.Polygons.City`, `INodeInfo`/`IInstancesNode`) — строит меш через `RegionMeshBuilder.Build` и материализует его через host instance maker (`MeshInstanceMaker`); превью пустое, `NodeInfo` = число треугольников. Тяжёлую сборку крутит в `PcgWorkerScheduler` и **пампит прогресс** (`PcgComputeSystem.ReportProgress` в цикле ожидания с шагом 250 мс): без этого ядро убивает ноду по watchdog «no progress for longer than 10 s» на крупных футпринтах (единый дорожный `UnionRegions` города квадратного километра считается дольше 10 с). Любой исполнитель с одним длинным `RunAsync` должен делать так же — иначе он необъяснимо «отваливается» на больших входах.
